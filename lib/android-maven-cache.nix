...
# Example:
#
#   mavenCacheTar = harbor-android.lib.findLocalMavenCache {
...
"harbor-android: findLocalMavenCache `sha256Path` must be a path or string";
assert lib.assertMsg (builtins.isPath hostPath || builtins.isString hostPath)
"harbor-android: findLocalMavenCache `hostPath` must be a path or string";
assert lib.assertMsg (builtins.isString name && name != "")
"harbor-android: findLocalMavenCache `name` must be a non-empty string";
assert lib.assertMsg (builtins.isBool recursive)
"harbor-android: findLocalMavenCache `recursive` must be a boolean"; let
  ...
  hash =
    ...
    then throw "harbor-android: findLocalMavenCache `${toString sha256Path}` is empty or contains no hash"
  ...
  normalizedHash =
    ...
    then throw "harbor-android: findLocalMavenCache `${toString sha256Path}` does not contain a valid SHA-256 hash"
    else
...
in
  assert hash == null || validHash || throw "harbor-android: findLocalMavenCache `${toString sha256Path}` does not contain a valid SHA-256 hash";
    if hash == null || !(builtins.pathExists hostPath)
    then null
    else
      assert lib.assertMsg (builtins.isPath hostPath || lib.hasPrefix "/" hostPathString)
      "harbor-android: findLocalMavenCache `hostPath` must be absolute when passed as a string";
        builtins.path {
          path = hostPath;
