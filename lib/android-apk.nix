...
#   packages.android-apk-debug = harbor-android.lib.mkAndroidApk {
...
in
  ...
  "harbor-android: mkAndroidApk `gradleModule` must look like \"app\" or \"test-peer\" with a leading colon";
  assert lib.assertMsg (builtins.elem mode ["debug" "release"])
  "harbor-android: mkAndroidApk `mode` must be \"debug\" or \"release\", got ${mode}";
  assert lib.assertMsg (rustToolchain != null)
  "harbor-android: mkAndroidApk requires `rustToolchain` — pass `(mkToolchain { ... }).rustToolchain`";
  assert lib.assertMsg (builtins.isList cargoFeatures && builtins.all (feature: builtins.isString feature && feature != "") cargoFeatures)
  "harbor-android: mkAndroidApk `cargoFeatures` must be a list of non-empty strings";
  assert lib.assertMsg (validRelativePath androidDir)
  "harbor-android: mkAndroidApk `androidDir` must be a non-empty relative path without `..`";
  assert lib.assertMsg (validRelativePath jniLibsDir)
  "harbor-android: mkAndroidApk `jniLibsDir` must be a non-empty relative path without `..`";
  assert lib.assertMsg (validRelativePath apkOutPath)
  "harbor-android: mkAndroidApk `apkOutPath` must be a non-empty relative path without `..`";
  assert lib.assertMsg (builtins.isString abi && abi != "")
  "harbor-android: mkAndroidApk `abi` must be a non-empty string";
  assert lib.assertMsg (builtins.isString ndkVersion && ndkVersion != "")
  "harbor-android: mkAndroidApk `ndkVersion` must be a non-empty string";
  assert lib.assertMsg (cargoNdkPlatform == null || (builtins.isInt cargoNdkPlatform && cargoNdkPlatform > 0))
  "harbor-android: mkAndroidApk `cargoNdkPlatform` must be null or a positive Android API level";
  assert lib.assertMsg (buildCommand == null || (builtins.isString buildCommand && buildCommand != ""))
  "harbor-android: mkAndroidApk `buildCommand` must be null or a non-empty string";
