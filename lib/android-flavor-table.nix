...
"harbor-android: mkAndroidFlavorTable requires a non-empty `flavors` attrset";
assert lib.assertMsg (builtins.hasAttr defaultFlavor flavors)
"harbor-android: mkAndroidFlavorTable defaultFlavor `${defaultFlavor}` is not in flavors";
assert lib.assertMsg (builtins.isList modes && modes != [])
"harbor-android: mkAndroidFlavorTable `modes` must be a non-empty list";
assert lib.assertMsg (builtins.elem defaultMode ["debug" "release"])
"harbor-android: mkAndroidFlavorTable `defaultMode` must be \"debug\" or \"release\"";
assert lib.assertMsg (cargoNdkPlatform == null || (builtins.isInt cargoNdkPlatform && cargoNdkPlatform > 0))
"harbor-android: mkAndroidFlavorTable `cargoNdkPlatform` must be null or a positive Android API level";
assert lib.assertMsg (gradleDeps == null)
"harbor-android: mkAndroidFlavorTable `gradleDeps` is not implemented; use `mavenCacheTar` plus `cargoVendorDir`";
assert lib.assertMsg (mavenCacheTar == null || cargoVendorDir != null)
"harbor-android: mkAndroidFlavorTable `mavenCacheTar` also requires `cargoVendorDir` from `craneLib.vendorCargoDeps`"; let
  ...
  normalizeFlavor = name: cfg:
    ...
    "harbor-android: mkAndroidFlavorTable flavor `${name}` requires cargoPkg"; let
    ...
    in
      ...
      "harbor-android: mkAndroidFlavorTable flavor `${name}` cargoPkg must be a non-empty string";
      assert lib.assertMsg (builtins.isString gradleModule && lib.hasPrefix ":" gradleModule)
      "harbor-android: mkAndroidFlavorTable flavor `${name}` gradleModule must look like \":app\"";
      assert lib.assertMsg (builtins.isList packageModes && packageModes != [])
      "harbor-android: mkAndroidFlavorTable flavor `${name}` packageModes must be a non-empty list";
      assert lib.assertMsg (builtins.all validMode packageModes)
      "harbor-android: mkAndroidFlavorTable flavor `${name}` packageModes may only contain \"debug\" or \"release\"";
      assert lib.assertMsg (builtins.isString devAppAttr && devAppAttr != "")
      "harbor-android: mkAndroidFlavorTable flavor `${name}` devAppAttr must be a non-empty string";
      assert lib.assertMsg (flavorCargoNdkPlatform == null || (builtins.isInt flavorCargoNdkPlatform && flavorCargoNdkPlatform > 0))
      "harbor-android: mkAndroidFlavorTable flavor `${name}` cargoNdkPlatform must be null or a positive Android API level"; {
      ...
      name: cfg:
        map (
          ...
          in
            ...
            "harbor-android: mkAndroidFlavorTable flavor `${name}` packageAttr must return a non-empty string"; {
              inherit attr;
              value = mkAndroidApk {
  ...
  in
    ...
    "harbor-android: mkAndroidFlavorTable produced duplicate ${kind} attribute names: ${lib.concatStringsSep ", " names}"; true;
